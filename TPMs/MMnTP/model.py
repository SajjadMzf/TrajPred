...
class MMnTP(nn.Module): 
    def __init__(self, batch_size, device, hyperparams_dict, parameters, drop_prob = 0.1):
        ...
        ''' 6. Manouvre Output '''
        if self.MAP_ENCODER:
            dec_man_fc1_in_size = 128 + self.in_seq_len*self.model_dim
        else:
            dec_man_fc1_in_size =  self.in_seq_len*self.model_dim
    ...
    def forward(self, x, y, y_mask):      
        ...
        man_pred = self.man_decoder_forward(encoder_out, map_encoder_out)
    ...
    def encoder_forward(self, x):
        ...
        map_data = x[:,:,self.input_dim:]
        x = x[:,:,:self.input_dim] 
        x = self.encoder_embedding(x)
        x = self.positional_encoder(x)
        encoder_out = self.transformer_encoder(x)
        if self.MAP_ENCODER:
            map_encoder_out = F.relu(self.map_ff(map_data.reshape(self.batch_size, -1)))
        else:
            map_encoder_out = 0
        return encoder_out, map_encoder_out
    
    def man_decoder_forward(self, encoder_out, map_encoder_out):
        man_dec_in = encoder_out.reshape(self.batch_size, self.in_seq_len*self.model_dim)
        if self.MAP_ENCODER:
