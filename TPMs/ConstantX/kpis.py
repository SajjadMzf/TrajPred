...
def ConstantX_kpis(p, kpi_input_dict, traj_min, traj_max, figure_name):
    '''
    1. NLL (TBD)
    2. error based 
...
def calc_rmse(p, traj_pred, traj_gt):
    ...
    rmse_table = np.zeros((prediction_ts,3))
    for ts in range(prediction_ts):
        ...
        rmse_table[ts,0] = np.sqrt(
            np.sum((cur_traj_pred-cur_traj_gt)**2)/cur_n_samples
            )
        rmse_table[ts,1] = np.sqrt(
            np.sum((cur_traj_pred[:,:,0]-cur_traj_gt[:,:,0])**2)/cur_n_samples
            )
        rmse_table[ts,2] = np.sqrt(
            np.sum((cur_traj_pred[:,:,1]-cur_traj_gt[:,:,1])**2)/cur_n_samples
            )
